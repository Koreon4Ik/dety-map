...
export default function AdminPage() {
  ...
  // Завантажуємо поточні локації безпечним способом
  useEffect(() => {
    const loadInitialData = async () => {
      // Виконуємо запит тільки в браузері, щоб уникнути помилок при збірці на Vercel
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-8 font-sans">
      <div className="max-w-5xl mx-auto">
        ...
        {status && (
          <div className="mb-6 p-4 bg-green-500/20 border border-green-500/50 rounded-xl text-green-400 text-sm font-bold animate-bounce">
            {status}
          </div>
        )}
                ...
                {locations.map((loc, index) => (
                  <tr key={index} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="p-4">
                      ...
                      >
                        {['МЦ', 'NGO', 'Освіта', 'Коворкінг', 'Волонтерство', 'Спорт', 'Культура'].map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                    </td>
                    <td className="p-4">
