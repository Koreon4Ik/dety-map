...
export default function Home() {
  ...
  const [userLocation, setUserLocation] = useState(null); // Стан для координат користувача
  ...
  const findMe = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        // Передаємо координати в стан, який ми потім прокинемо в мапу
        setUserLocation([latitude, longitude]);
      },
      () => {
        alert("Не вдалося отримати доступ до вашого місцезнаходження");
      }
    );
  };
  ...
  return (
    <div className="h-screen w-full bg-slate-950 relative overflow-hidden font-sans text-white">
            ...
            {['ВСІ', 'МЦ', 'NGO', 'ОСВІТА', 'КОВОРКІНГ', 'СПОРТ'].map(cat => (
              <button
                ...
                className={`px-5 py-3.5 rounded-2xl text-[11px] font-black uppercase transition-all whitespace-nowrap tracking-wider active:scale-95 ${
                  filter === cat ? 'bg-yellow-400 text-black shadow-xl shadow-yellow-400/30' : 'bg-white/5 text-slate-400 hover:bg-white/10 hover:text-white'
                }`}
              >
                {cat}
              </button>
      ...
      {/* --- МОДАЛКА ПРО НАС (БЕЗ ЗМІН) --- */}
      {isAboutOpen && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-xl animate-fade-in">
          <div className="bg-slate-900 border border-white/10 rounded-[56px] p-12 md:p-16 max-w-4xl w-full relative shadow-2xl shadow-black/70 overflow-hidden">
            <button onClick={() => setIsAboutOpen(false)} className="absolute top-8 right-8 p-3.5 bg-white/5 rounded-full hover:bg-white/10 transition-all text-slate-400 hover:text-white z-10">
              <X size={28} />
            </button>
            ...
            <div className="text-slate-400 space-y-6 leading-relaxed text-xl font-medium relative z-10 max-w-3xl">
              <p>DETY MAP — це проект для молодих та активних. Ми зібрали всі круті локації твого міста на одній інтерактивній мапі.</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="h-full w-full">
        {/* Передаємо center={userLocation} в компонент мапи */}
        <MapCustom 
          points={filteredData} 
          center={userLocation} 
      ...
      <div className="absolute bottom-10 right-10 z-[1000] flex flex-col gap-4">
        {/* НОВА КНОПКА ГЕОЛОКАЦІЇ */}
        <button 
          onClick={findMe}
          className="bg-white text-black p-5 rounded-3xl hover:bg-yellow-400 transition-all shadow-2xl flex items-center justify-center active:scale-90"
          title="Де я?"
        >
          <Navigation size={28} fill="currentColor" />
        </button>

        <Link href="/login" className="bg-slate-900 border border-white/10 text-white p-5 rounded-3xl hover:bg-white hover:text-black transition-all shadow-xl group flex items-center justify-center">
      ...
      <style jsx global>{`
        ...
        @keyframes pulse-slow { 0%, 100% { transform: scale(1); filter: brightness(1); } 50% { transform: scale(0.97); filter: brightness(1.1); } }
